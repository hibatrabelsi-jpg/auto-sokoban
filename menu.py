...
SOLVER_COLOR = (80, 150, 255)
SOLVER_HOV   = (120, 180, 255)


def run_menu(screen):
    """
    Affiche le menu de sélection.
    Retourne (difficulty_name, level_path, mode) où mode = "play" ou "solver"
    Retourne None si quitter.
    """
    ...
    while True:
        ...
        play_rect   = pygame.Rect(W // 2 - 230, 370, 200, 58)
        solver_rect = pygame.Rect(W // 2 + 30,  370, 200, 58)
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                ...
                if event.key in (pygame.K_RETURN, pygame.K_SPACE):
                    return difficulties[sel_diff], levels[sel_lvl], "play"
                if event.key == pygame.K_s:
                    return difficulties[sel_diff], levels[sel_lvl], "solver"
        ...
        # Bouton SOLVEUR
        sol_hov = solver_rect.collidepoint(mouse_pos)
        pygame.draw.rect(screen, SOLVER_HOV if sol_hov else SOLVER_COLOR,
                         solver_rect, border_radius=12)
        pygame.draw.rect(screen, (20, 80, 180), solver_rect, 3, border_radius=12)
        sol_lbl = font_sub.render("SOLVEUR", True, (255, 255, 255))
        screen.blit(sol_lbl, sol_lbl.get_rect(center=solver_rect.center))

        info = font_hint.render(
            f"Selection : {difficulties[sel_diff]}  -  Niveau {sel_lvl + 1}",
            True, (160, 200, 160))
        screen.blit(info, info.get_rect(center=(W // 2, 455)))

        hint = font_hint.render(
            "Entree = Jouer   S = Solveur   Q = Quitter",
            True, (100, 100, 140))
        screen.blit(hint, hint.get_rect(center=(W // 2, 510)))
