...
def run_game(screen, difficulty, level_path, auto_solve=False):
    """
    Lance une partie. Si auto_solve=True, lance le solveur dès le départ.
    Retourne : "menu" | "next" | "quit"
    """
    ...
    mode_label = "Solveur IA" if auto_solve else difficulty
    pygame.display.set_caption(f"Sokoban – {level_name}  [{mode_label}]")
    ...
    def _render(show_deadlock=False):
        ...
        draw_hud(screen, font_hud, moves_count, best_score, level_name, mode_label, W)
    ...
    # ── Lancement automatique du solveur ──────────────────────
    if auto_solve:
        _render()
        ...
        if solution:
            for move in solution:
                # Pomper les events pour ne pas bloquer la fenêtre
            ...
            # Pas de solution : afficher un message et retourner au menu
            screen.fill((12, 12, 28))
            font_err = pygame.font.SysFont("Arial", 28, bold=True)
            msg = font_err.render("Aucune solution trouvee pour ce niveau.", True, (255, 80, 80))
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                ...
                if event.key == pygame.K_r:
                    ...
                    is_new_record = False
                    undo_stack.clear()
                    # Si mode solveur, relancer directement
                    if auto_solve:
                        return run_game(screen, difficulty, level_path, auto_solve=True)
                ...
                if event.key == pygame.K_a and not auto_solve:
                    solving  = True
                    solution = a_star_solver(deepcopy(matrix))
                    if solution:
                        for move in solution:
...
def main():
    ...
    while True:
        ...
        difficulty, level_path, mode = result
        levels     = DIFFICULTY_LEVELS[difficulty]
        idx        = levels.index(level_path)
        auto_solve = (mode == "solver")

        while idx < len(levels):
            outcome = run_game(screen, difficulty, levels[idx], auto_solve=auto_solve)
            if outcome == "quit":
                pygame.quit()
                return
            elif outcome == "menu":
